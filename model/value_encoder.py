...
class ValueEncoder(nn.Module):
    ...
    def __init__(self, d_model: int, frozen_lm_dim: int, n_fourier_feats: int, fourier_learnable: bool,
        ...
        super().__init__()
        if numeric_path_beta and numeric_path_film:
            raise ValueError("numeric_path_beta and numeric_path_film are mutually exclusive.")
    ...
    def forward(self, node_types: list[str], raw_values: list, lm_embeddings: Tensor = None,
        ...
        lm_embeddings: 如果该 batch 中有 string，需在外部先用 FrozenLM 算出 (N_str, lm_dim) 传入。
                       如果没有 string，可传 None。
        path_context: 可选 (N, d_model) 路径上下文；启用 numeric_path_beta/film 时只调制 number token。
        返回: (N, d_model) 的特征表示
        """
        N = len(node_types)
        ...
        if num_indices:
            ...
            if self.numeric_path_beta or self.numeric_path_film:
                if path_context is None:
                    raise ValueError("numeric_path_beta/film requires path_context.")
