...
class ValueEncoderPathFilmTests(unittest.TestCase):
    def test_zero_initialized_path_beta_preserves_numeric_encoding(self):
        ...
        beta = ValueEncoder(
            ...
            n_fourier_feats=4,
            fourier_learnable=False,
            numeric_path_beta=True,
        )
        beta.mantissa_encoder.load_state_dict(base.mantissa_encoder.state_dict())
        beta.exponent_embed.load_state_dict(base.exponent_embed.state_dict())
        ...
        beta_out = beta(node_types, raw_values, path_context=path_context)

        self.assertTrue(torch.allclose(base_out, beta_out, atol=1e-6))
    ...
    def test_path_beta_requires_path_context_for_numbers(self):
    ...
    def test_path_beta_and_film_are_mutually_exclusive(self):
